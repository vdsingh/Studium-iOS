...
extension AlertTimeSelectingForm where Self: Coordinated {
    ...
    func showAlertTimesSelectionViewController() {
        ...
        if let coordinator = coordinator as? AlertTimesSelectionShowingCoordinator {
            ...
            Log.s(
                AlertTimeSelectingFormError.failedToOptionalCast,
                additionalDetails: "Tried to show AlertTimesSelection Flow but the coordinator is not AlertTimesSelectionShowingCoordinator. Coordinator: \(String(describing: self.coordinator))"
            )
        }
    }
}
