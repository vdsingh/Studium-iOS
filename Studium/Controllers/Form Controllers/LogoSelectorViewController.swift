...
import UIKit
protocol LogoStorer {
    var systemImage: SystemIcon { get set }
...
extension LogoSelectorViewController: UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        delegate?.systemImage = logos[indexPath.row]
...
//        var logoNames = systemCourseLogoNames + systemHabitLogoNames;
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: LogoCollectionViewCell.reuseIdentifier, for: indexPath) as! LogoCollectionViewCell
        cell.setImage(image: logos[indexPath.row].createImage(), tintColor: color )
        return cell
    }
}
